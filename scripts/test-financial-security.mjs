...
try {
  ...
  await run(['--conditions=react-server', '--import', 'tsx', '--test', ...(process.env.TEST_NAME_PATTERN ? ['--test-name-pattern', process.env.TEST_NAME_PATTERN] : []), process.argv.includes('--workspace-design') ? 'tests/workspace-design.browser.test.mjs' : process.argv.includes('--hardening') ? 'tests/security-hardening.test.ts' : process.argv.includes('--email-browser') ? 'tests/email.browser.test.mjs' : process.argv.includes('--email') ? 'tests/email.test.ts' : process.argv.includes('--support-browser') ? 'tests/support.browser.test.mjs' : process.argv.includes('--support') ? 'tests/support.test.ts' : process.argv.includes('--imports') ? 'tests/import.browser.test.mjs' : process.argv.includes('--chat-components') ? 'tests/chat-components.browser.test.mjs' : process.argv.includes('--admin') ? 'tests/admin.browser.test.mjs' : process.argv.includes('--components') ? 'tests/component-library.browser.test.mjs' : shellOnly ? 'tests/shell-http.test.ts' : 'tests/financial-security.test.ts']);
