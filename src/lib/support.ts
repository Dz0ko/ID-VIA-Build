...
export async function supportSnapshot(id: string, user: SessionUser, before?: string): Promise<{ thread: { id: string; status: string; supportTier: string; assignedTo?: string | null; botPending: boolean; updatedAt: Date; user?: { name: string | null; email: string } }; messages: { id: string; threadId: string; role: string; content: string; authorName: string | null; clientId: string; createdAt: Date }[]; hasMore: boolean }> {
  const thread = await db.supportThread.findFirst({ where: { id, ...(canManageSupport(user.role) ? {} : { userId: user.id }) }, include: { user: { select: { name: true, email: true, plan: true } } } });
  ...
  return { thread: { id: thread.id, status: thread.status, supportTier: supportTier(thread.user.plan), assignedTo: canManageSupport(user.role) ? thread.assignedTo : undefined, botPending: !!thread.botUntil && thread.botUntil > new Date(), updatedAt: thread.updatedAt, ...(canManageSupport(user.role) ? { user: thread.user } : {}) }, messages: messages.slice(0, 100).reverse(), hasMore };
}
...
export async function sendSupport(id: string, user: SessionUser, input: { message?: string; clientId: string; action?: string; asManager?: boolean }) {
  const result = await db.$transaction(async tx => {
    ...
    if (!thread || (!canManageSupport(user.role) && thread.userId !== user.id)) throw new SupportError("Conversation not found.", 404);
    if (await tx.supportMessage.findUnique({ where: { threadId_clientId: { threadId: id, clientId: input.clientId } } })) return null;
    const staff = canManageSupport(user.role) && (thread.userId !== user.id || input.asManager === true);
    const action = input.action ?? "message";
    if (staff && thread.assignedTo && thread.assignedTo !== user.id && ["message", "close"].includes(action)) throw new SupportError("Another manager owns this conversation. Claim it before replying.", 409);
    if (action === "claim" || action === "close") {
      if (!staff) throw new SupportError("Support staff access required.", 403);
    ...
    const handoff = action === "handoff" || (!staff && /\b(human|manager|real person|support agent|real man|menadzer|menadžer)\b|менаџер|човек/i.test(input.message ?? ""));
    const stale = !!thread.botUntil && thread.botUntil < new Date();
    if (!staff && thread.botUntil && !stale && !handoff) throw new SupportError("Please wait for the current support answer.", 409);
    if (action === "message") {
      if (!input.message?.trim()) throw new SupportError("Write a message.");
      await tx.supportMessage.create({ data: { threadId: id, clientId: input.clientId, role: staff ? "manager" : "user", authorName: staff ? (user.name || "Support manager") : null, content: input.message.trim() } });
    }
    const nextStatus = staff ? "HUMAN" : handoff || stale ? "WAITING" : thread.status === "CLOSED" ? "BOT" : thread.status;
    const turns = thread.status === "CLOSED" ? 0 : thread.botTurns;
    const escalate = !staff && (handoff || stale || (nextStatus === "BOT" && turns >= 3));
    const token = !staff && !escalate && nextStatus === "BOT" ? randomUUID() : null;
    await tx.supportThread.update({ where: { id }, data: { status: escalate ? "WAITING" : nextStatus, assignedTo: staff ? user.id : nextStatus === "BOT" ? null : thread.assignedTo, botTurns: token ? turns + 1 : turns, botToken: token, botUntil: token ? new Date(Date.now() + 45000) : null } });
