...
export type PortalDenied = { status: 404 | 410 | 401; message: string };
...
export async function checkPortalLink(token: string, password: string | null): Promise<PortalDenied | null> {
  ...
  if (link.password && !safeEqual(link.password, password)) return { status: 401, message: "Password required." };
  return null;
}
