...
export async function GET(_req: Request, ctx: RouteContext<"/s/[slug]">) {
  ...
  const project = await db.project.findUnique({ where: { slug }, select: { id: true, kind: true, publishedHtml: true, status: true, name: true } });
  if (project?.kind === "app" && project.status === "PUBLISHED") {
    ...
    try { const url = new URL(deployment?.url ?? ""); if (url.protocol === "https:" && url.hostname.endsWith(".vercel.app")) return Response.redirect(url, 307); } catch { /* No valid deployment. */ }
  }
