...
type Thread = { id: string; status: string; supportTier: string; user: { name: string | null; email: string }; messages: { content: string; role: string }[] };
export function SupportInbox({ adminId }: { adminId: string }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        if (!document.hidden) {
          const response = await fetch(`/api/admin/support?${new URLSearchParams({ status, tier, q: query, page: String(page) })}`, { signal: controller.signal, cache: "no-store" });
  ...
  return <div className="grid grid-cols-[280px_minmax(0,1fr)] gap-5 h-[calc(100dvh-190px)] min-h-[500px]">
    <aside className="flex flex-col min-h-0 gap-3"><input className="input w-full" aria-label="Search support customers" placeholder="Search name or email…" value={query} onChange={event => { setQuery(event.target.value); setPage(1); }} /><select className="input w-full" aria-label="Conversation status" value={status} onChange={event => { setStatus(event.target.value); setPage(1); }}><option value="">All conversations</option><option value="WAITING">Waiting for a manager</option><option value="HUMAN">With a manager</option><option value="BOT">Automated support</option><option value="CLOSED">Resolved</option></select>
      <select className="input w-full" aria-label="Support tier" value={tier} onChange={event => { setTier(event.target.value); setPage(1); }}><option value="">All tiers · premium first</option><option value="premium">Premium · Max & Agency</option><option value="priority">Priority · Pro</option><option value="standard">Standard</option></select>
      {error && <p role="alert" className="text-xs text-red-400">{error}</p>}
      <div className="flex-1 overflow-y-auto space-y-2">{threads.map(thread => <button key={thread.id} onClick={() => setSelected(thread.id)} aria-pressed={selected === thread.id} className={selected === thread.id ? "w-full text-left rounded-xl border border-signal bg-graphite p-3" : "w-full text-left rounded-xl border border-graphite p-3 hover:bg-graphite"}><p className="text-sm truncate">{thread.user.name || thread.user.email}</p><p className="text-xs text-signal-soft mt-1">{thread.supportTier === "premium" ? "Premium" : thread.supportTier === "priority" ? "Priority" : "Standard"}</p><p className="text-xs text-ash mt-1">{thread.status === "WAITING" ? "Waiting for you" : thread.status === "HUMAN" ? "With a manager" : thread.status === "CLOSED" ? "Resolved" : "Automated support"}</p><p className="text-xs text-fog mt-2 truncate">{thread.messages[0]?.content || "New conversation"}</p></button>)}{!threads.length && <p className="text-sm text-ash p-4">No conversations in this view.</p>}</div>
