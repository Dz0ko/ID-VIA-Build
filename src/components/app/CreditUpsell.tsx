...
import { CREDIT_NOTICE_EVENT, PLAN_NOTICE_EVENT, type PlanNotice } from "@/lib/credit-notice";
...
export function CreditUpsell({ credits, plan }: { credits: number; plan: PlanId }) {
  ...
  // A plan limit (locked agent, feature, project count) uses the same dialog with its own wording.
  ...
  useEffect(() => {
    ...
    function showPlan(event: Event) {
      setPlanNotice((event as CustomEvent<PlanNotice>).detail);
      if (!dialog.current?.open) dialog.current?.showModal();
    }
    window.addEventListener(CREDIT_NOTICE_EVENT, show); window.addEventListener(PLAN_NOTICE_EVENT, showPlan);
    return () => { window.removeEventListener(CREDIT_NOTICE_EVENT, show); window.removeEventListener(PLAN_NOTICE_EVENT, showPlan); };
  }, [credits]);
  useEffect(() => {
  ...
  if (planNotice) {
    const target = planNotice.minPlanName ?? (planNotice.minPlan ? PLANS[planNotice.minPlan].name : null);
    const title = planNotice.code === "PROJECT_LIMIT" ? "You’ve reached your plan’s project limit." : target ? `This needs the ${target} plan.` : "This isn’t included in your plan.";
    return <dialog ref={dialog} className="credit-dialog m-auto w-[calc(100%-2rem)] max-w-xl rounded-2xl border border-graphite bg-ink text-paper p-7 backdrop:bg-black/75" aria-labelledby="credit-dialog-title">
      <div className="flex items-center justify-between"><span className="label text-signal-soft">Unlock more</span><button className="btn btn-ghost btn-sm" aria-label="Close plan options" onClick={() => dialog.current?.close()}><X size={18} /></button></div>
      <h2 id="credit-dialog-title" className="text-2xl font-semibold mt-4">{title}</h2>
      <p className="text-sm text-ash mt-3 leading-relaxed">{planNotice.message ?? "Upgrade to continue."} You are on the {PLANS[plan].name} plan. Your projects and conversations are saved; nothing was charged.</p>
      <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-5 border-t border-graphite"><button className="btn btn-ghost btn-sm" onClick={() => dialog.current?.close()}>Maybe later</button><Link href="/app/settings#plans" onClick={() => dialog.current?.close()} className="btn btn-primary">{target ? `Upgrade to ${target}` : "Compare plans"} →</Link></div>
      <p className="text-[11px] text-ash mt-4">Checkout is handled by Whop. The upgrade applies as soon as payment is confirmed.</p>
    </dialog>;
  }
  return <dialog ref={dialog} className="credit-dialog m-auto w-[calc(100%-2rem)] max-w-xl rounded-2xl border border-graphite bg-ink text-paper p-7 backdrop:bg-black/75" aria-labelledby="credit-dialog-title">
