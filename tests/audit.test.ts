...
test("app audit does not manufacture eleven HTML errors from an unused empty document", () => {
  const result = auditProject({ kind: "app", html: "", files: [{ path: "/app/page.tsx", content: "export default function Page() { return <main><h1>Hello</h1></main> }" }] });
...
test("app source findings identify files and disappear only when the source changes", () => {
  ...
  assert.equal(auditProject(project).issues.length, 2);
  project.files[0].content = '<html lang="en"><img src="a.png" alt="" /></html>';
  assert.equal(auditProject(project).issues.length, 0);
});
